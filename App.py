...
# ====================== TABS ======================
tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "🎯 Targets & Levels", "📈 Sensitivity", "📉 Live Chart"])

with tab1:
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("MCX Actual", f"₹{mcx_actual:,}")
    with col2:
        st.metric("COMEX Silver", f"${comex:.2f}")
    with col3:
        st.metric("Gold Price", f"${gold:.0f}")
    with col4:
        st.metric("Crude Oil", f"${oil:.2f}")

    st.write(f"**Gold-Silver Ratio**: {gsr}:1  **Silver-Oil Ratio**: {silver_oil_ratio}")
...
with tab2:
    ...
    with colB:
        ...
        hard_stop = round(vc_pmi_comex["daily_buy2"] * 0.95 * scaling_factor * 32.1507 * usdinr, 0)
        st.write(f"**Buy Zone**: ₹{buy_low:,} – ₹{buy_high:,}")
        st.write(f"**Hard Stop**: Below ₹{hard_stop:,}")
        st.write("**₹2,20,000 Probability**: 12% (tail risk)")

with tab3:
    st.subheader("Gold Price / GSR Sensitivity")
    gsr_table = {
        "65:1 (current)": round(gold / 65 * 32.1507 * usdinr, 0),
        "70:1": round(gold / 70 * 32.1507 * usdinr, 0),
        "80:1": round(gold / 80 * 32.1507 * usdinr, 0),
        "90:1": round(gold / 90 * 32.1507 * usdinr, 0)
    }
    for ratio, target in gsr_table.items():
        st.write(f"GSR at {ratio} → Silver target **₹{target:,}**")
    ...
    for oil_price, silver_target in oil_table.items():
        st.write(f"Oil at {oil_price} → Silver target **${silver_target}**")
...
st.caption("Auto-refreshes every 60 seconds • All targets dynamically scaled to your actual MCX price • Full consolidation of VC PMI + all experts data • Built exclusively for Satbeer • Kolkata")
